...
def test_web_model_choices_are_limited_to_the_supported_runtime():
    ...
    # The dropdown and the allowlist have to agree, or the UI offers a model the
    # API then refuses.
    html = (PACKAGE_DIR / "templates" / "index.html").read_text(encoding="utf-8")
    offered = set(re.findall(r'<option value="([^"]+)">[^<]*纠错[^<]*</option>', html))
    assert offered <= WEB_ALLOWED_MODELS, sorted(offered - WEB_ALLOWED_MODELS)
    assert WEB_ALLOWED_MODELS - {"rule-based"} <= offered
