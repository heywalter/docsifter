...
For how good a correction model is, use its own evaluation rather than this
corpus. On the pycorrector leaderboard — F1 across SIGHAN-2015, EC-LAW and MCSC,
measured on a Tesla V100 — the three models DocSifter offers score:

| Model | Avg | SIGHAN-2015 | EC-LAW | MCSC | QPS |
| --- | --- | --- | --- | --- | --- |
| `shibing624/chinese-text-correction-1.5b` | 0.6802 | 0.3032 | 0.7846 | 0.9529 | 6 |
| `twnlp/ChineseErrorCorrector3-4B` | 0.8521 | 0.6340 | 0.9360 | 0.9864 | 5 |
| `shibing624/chinese-text-correction-7b` | 0.8225 | 0.4917 | 0.9798 | 0.9959 | 3 |

CPU inference, which is what DocSifter defaults to, is well below those QPS
figures. The full table, with the other models compared alongside, is on each
[model card](https://huggingface.co/shibing624/chinese-text-correction-1.5b) and
in [ChineseErrorCorrector](https://github.com/TW-NLP/ChineseErrorCorrector),
which publishes the same evaluation.

Those numbers describe a model. What DocSifter adds around one — the rule
layer, whitelist and syntax protection, the false-positive filter — is what the
corpus below illustrates, and it was measured with the 1.5B. The 4B and the 7B
have not been run against this corpus.
